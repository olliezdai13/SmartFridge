...
- Authentication: start a session via `/api/auth/signup` or `/api/auth/login`; auth is cookie-based and no shared-secret header is needed. With curl, use a cookie jar (`-c/-b`) between calls.
- `POST /api/snapshot` — multipart upload with `image` (and optional `prompt`); returns `202` with `snapshot_id`, bucket/key, and initial status. Poll the `snapshots` row to track progress. Example after login:
  ```bash
